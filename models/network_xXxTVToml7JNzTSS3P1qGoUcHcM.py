# --PROMPT LOG--

import torch.nn as nn
from stable_baselines3.common.policies import ActorCriticPolicy

# --OPTION--
# -- NOTE --
# Note: The class GenePolicy inherits from ActorCriticPolicy (Stable-Baselines3).
# It must always accept *args and **kwargs and pass them to super().__init__().
# get_policy_kwargs() must return a dict with "policy_class" key.
# get_ppo_kwargs() must return a dict of valid PPO hyperparameters.# -- NOTE --
import torch.nn as nn

# -- NOTE --
# Note: The class GenePolicy inherits from ActorCriticPolicy (Stable-Baselines3).
# It must always accept *args and **kwargs and pass them to super().__init__().
# get_policy_kwargs() must return a dict with "policy_class" key.
# get_ppo_kwargs() must return a dict of valid PPO hyperparameters.
# -- NOTE --

# ===============================
# === Architecture Gene Space ===
# ===============================

# Modified Hidden Layers
HIDDEN_PI = [128, 128]  # Increased size for potentially better feature extraction
HIDDEN_VF = [128, 128]  # Symmetrical increase for value function
ACTIVATION = nn.ReLU     # Changed to ReLU for potentially faster training

class GenePolicy(ActorCriticPolicy):
    def __init__(self, *args, **kwargs):
        super().__init__(
            *args,
            **kwargs,
            activation_fn=ACTIVATION,
            net_arch=dict(
                pi=HIDDEN_PI,
                vf=HIDDEN_VF
            ),
        )

def get_policy_kwargs():
    return {
        "policy_class": GenePolicy
    }

# Example of modifying PPO hyperparameters
def get_ppo_kwargs():
    return {
        "n_steps": 2048,         # Number of steps to run for each environment per update
        "ent_coef": 0.01,        # Entropy coefficient for the loss calculation
        "learning_rate": 0.00025, # The learning rate, it can be a function
        "batch_size": 64,        # Minibatch size for each gradient update
        "clip_range": 0.2,       # Clipping parameter, paper default value
    }

# --OPTION--

# ===============================
# === PPO Hyperparameter Gene ===
# ===============================

def get_ppo_kwargs():
    return dict(
        learning_rate=3e-4,
        gamma=0.99,
        gae_lambda=0.95,
        clip_range=0.2,
        ent_coef=0.0,
        vf_coef=0.5,
        max_grad_norm=0.5,
        n_steps=2048,
        batch_size=64,
        n_epochs=10,
        normalize_advantage=True,
        verbose=0,
    )
