    ...
    **kwargs: Arbitrary keyword arguments.
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.pi_mlp = nn.Sequential(
            nn.Linear(self.observation_space.shape[0], HIDDEN_PI[0]),
            ACTIVATION(),
            nn.Linear(HIDDEN_PI[0], HIDDEN_PI[1]),
            ACTIVATION(),
            nn.Linear(HIDDEN_PI[1], self.action_space.shape[0]),
            nn.Tanh()  # Use Tanh activation for the output layer
        )
        
        # Modify the value function network
        self.vf_mlp = nn.Sequential(
            nn.Linear(self.observation_space.shape[0], HIDDEN_VF[0]),
            ACTIVATION(),
            nn.Linear(HIDDEN_VF[0], HIDDEN_VF[1]),
            ACTIVATION(),
            nn.Linear(HIDDEN_VF[1], 1)  # Output a single value
        )
